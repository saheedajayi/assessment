                    ...
                    <div className="p-6">
                        <SheetHeader className="space-y-4">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1 space-y-2">
                                    <SheetTitle className="text-xl leading-tight">{sanitizeText(recommendation.title)}</SheetTitle>
                                    <div className="flex flex-wrap items-center gap-4">
                                        <div className="flex items-center gap-2">
                                            <span className="text-sm font-medium">Value Score:</span>
                                            <ValueScore score={valueScore} />
                                        ...
                                        {recommendation.provider.map((provider, idx) => (
                                            <div key={idx} className="flex items-center gap-2">
                                                <ProviderIcon provider={provider} />
                                                <span className="text-sm font-medium">{sanitizeText(getProviderName(provider))}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="mt-6 space-y-6">
                            ...
                            {/* Description */}
                            <div>
                                <p className="text-muted-foreground leading-relaxed">{sanitizeText(recommendation.description)}</p>
                            </div>
                        ...
                        <span className="font-semibold">
                          {sanitizeText(recommendation.impactAssessment.mostImpactedScope.name)}
                        </span>
                                                <span className="font-semibold">{recommendation.impactAssessment.mostImpactedScope.count}</span>
                                            </div>
                                            <p className="text-xs text-muted-foreground">
                                                ({sanitizeText(recommendation.impactAssessment.mostImpactedScope.type)})
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="space-y-2">
                                    {recommendation.furtherReading.map((reading, idx) => (
                                        <a
                                            key={idx}
                                            href={sanitizeUrl(reading.href)}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="flex items-center gap-2 text-primary hover:underline"
                                        >
                                            <span>{sanitizeText(reading.name)}</span>
                                            <ExternalLink className="h-4 w-4" />
                                        </a>
                                    ))}
                                </div>
